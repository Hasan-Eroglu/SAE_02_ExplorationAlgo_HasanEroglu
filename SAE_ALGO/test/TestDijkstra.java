...
public class TestDijkstra {
    ...
    @Test
    public void testMemeCheminQueBellmanFord() {
        GrapheListe g = creerGraphe();
        Dijkstra d = new Dijkstra();
        BellmanFord bf = new BellmanFord();

        Valeurs vD = d.resoudre(g, "A");
        Valeurs vBF = bf.resoudre(g, "A");

        // Les deux algorithmes doivent donner le même coût
        assertEquals(vBF.getValeur("C"), vD.getValeur("C"), 0.001);
        assertEquals(vBF.getValeur("B"), vD.getValeur("B"), 0.001);
        assertEquals(vBF.getValeur("E"), vD.getValeur("E"), 0.001);
    }
}